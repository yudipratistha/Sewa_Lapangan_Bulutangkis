                            ...
                            <div class="col-xl-12 xl-100 col-lg-12 box-col-12">
                                <div class="card">
                                    <div class="card-header">
                                        ...
                                        <h5 class="pull-left">Manajemen Libur Lapangan</h5>
                                    </div>
                                    <div class="card-body">
                                        ...
                                        <div class="table-responsive">
                                            <table class="display datatables" id="data-harga-promo-per-jam">
                                                <thead>
                                                    <tr>
                                                        <th>Libur Dari Tanggal</th>
                                                        <th>Libur Sampai Tanggal</th>
        ...
        <div class="modal-dialog modal-dialog-centered modal-md" role="document">
            <div class="modal-content">
                <div class="modal-header text-center d-block">
                    <h4 class="modal-title ">Tambah Libur Lapangan</h3>
                    ...
                    <form id="data-harga-promo-perjam" action="" method="POST">
                        ...
                        <div class="form-payment-method">
                            <div class="col-md-12 payment-method-name-div">
                                <label class="form-label" for="">Tanggal Libur Berlaku Dari & Sampai</label>
                ...
                <div class="modal-footer">
                    <button type="button" onClick="addLiburLapangan()" class="btn btn-success">Tambah Data Libur Lapangan</button>
                ...
                <div class="card-body">
                    <form id="data-harga-promo-perjam-edit" action="" method="POST">
                        ...
                        <div class="form-payment-method">
                            ...
                            <input id="libur-lapangan-id" type="hidden" name="libur_lapangan_id" value="" >
                        </div>
                    </form>
                </div>
                <div class="modal-footer">
...
<script>
    ...
    $('#tanggal').on('apply.daterangepicker', function(ev, picker) {
        ...
        tglLiburDari= picker.startDate.format('DD-MM-YYYY');
        tglLiburSampai= picker.endDate.format('DD-MM-YYYY');
    });
    ...
    var tableDataHargaPromoPerJam = $('#data-harga-promo-per-jam').DataTable({
        ...
        ajax: {
            type: "POST",
            url: "{{route('pemilikLapangan.getDataLiburLapangan')}}",
            dataType: "json",
        ...
        initComplete:function( settings, json){
            ...
            $('#data-harga-promo-per-jam tbody').on('click', "#restore-data-harga-per-jam", function() {
                ...
                restoreDataHargaPerJam(tableDataHargaPromoPerJam.row(row).data().libur_lapangan_id);
            });

            $('#data-harga-promo-per-jam tbody').on('click', "#edit-data-harga-per-jam", function() {
                let row = $(this).parents('tr')[0];
                editDataHargaPerJam(tableDataHargaPromoPerJam.row(row).data().libur_lapangan_id);
            });

            $('#data-harga-promo-per-jam tbody').on('click', "#delete-data-harga-per-jam", function() {
                let row = $(this).parents('tr')[0];
                deleteDataHargaPromoPerJam(tableDataHargaPromoPerJam.row(row).data().libur_lapangan_id);
            });

            $('#data-harga-promo-per-jam tbody').on('click', "#destroy-data-harga-per-jam", function() {
                let row = $(this).parents('tr')[0];
                destroyDataHargaPromoPerJam(tableDataHargaPromoPerJam.row(row).data().libur_lapangan_id);
            });
        }
    });

    function addLiburLapangan(){
        swal.fire({
            ...
            preConfirm: (login) => {
                return $.ajax({
                    type: "POST",
                    url: "{{route('pemilikLapangan.createLiburLapangan')}}",
                    data: {"_token": "{{ csrf_token() }}", 'tgl_libur_dari': tglLiburDari, 'tgl_libur_sampai': tglLiburSampai},
        ...
        swal.fire({
            ...
            preConfirm: (login) => {
                return $.ajax({
                    ...
                    data: {"_token": "{{ csrf_token() }}", "libur_lapangan_id" : liburLapanganId},
                    success: function(data) {
                        var request = 'success';
                    },
                    error: function(xhr, status, error){
    ...
    function destroyDataHargaPromoPerJam(liburLapanganId){
        swal.fire({
            ...
            preConfirm: (login) => {
                return $.ajax({
                    type: "POST",
                    url: "{{route('pemilikLapangan.destroyLiburLapangan')}}",
                    data: {"_token": "{{ csrf_token() }}", "libur_lapangan_id" : liburLapanganId},
                    success: function(data) {
                        var request = 'success';
                    },
    ...
    function editDataHargaPerJam(liburLapanganId){
        $.ajax({
            type: "POST",
            url: "{{route('pemilikLapangan.editLiburLapangan')}}",
            data: {"_token": "{{ csrf_token() }}", "libur_lapangan_id" : liburLapanganId},
            success: function(data) {
                tglLiburDari = data.tgl_libur_dari;
                tglLiburSampai = data.tgl_libur_sampai;

                $('#edit-tanggal').val(data.tgl_libur_dari.split('-').reverse().join('-')+' - '+data.tgl_libur_sampai.split('-').reverse().join('-'));
                $("#edit-tanggal").data('daterangepicker').setStartDate(data.tgl_libur_dari.split('-').reverse().join('-'));
                $("#edit-tanggal").data('daterangepicker').setEndDate(data.tgl_libur_sampai.split('-').reverse().join('-'));
                $('#libur-lapangan-id').val(data.libur_lapangan_id);
    ...
    function updateDataHargaPerJam(){
        swal.fire({
            ...
            preConfirm: (login) => {
                ...
                return $.ajax({
                    type: "POST",
                    url: "{{route('pemilikLapangan.updateLiburLapangan')}}",
                    data: {"_token": "{{ csrf_token() }}", 'libur_lapangan_id': $('#libur-lapangan-id').val(), 'tgl_libur_dari': tglLiburDari, 'tgl_libur_sampai': tglLiburSampai},
                    success: function(data) {
                        var request = 'success';
                        tglLiburDari= null;
                        tglLiburSampai= null;
